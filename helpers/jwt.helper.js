...
module.exports = {
  signAccessToken: (userId, user_type) => {
    return new Promise((resolve, reject) => {
      const payload = { id: userId ,type:user_type};
  ...
  signRefreshToken: (userId, user_type) => {
    return new Promise((resolve, reject) => {
      const payload = { id: userId ,type:user_type };
      ...
      const token = jwt.sign(payload, secret, options);
      client.SET(userId, token,"EX", process.env.REDIS_EXPIRY, (err, reply) => {
        if (err) {
